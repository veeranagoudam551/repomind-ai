...
matching chunks) are implemented standalone as of Day 16 via
`POST /repositories/{id}/search`, ahead of the "Build Context" → LLM
steps that need the LLM provider abstraction and conversation/message
persistence (a later phase).
